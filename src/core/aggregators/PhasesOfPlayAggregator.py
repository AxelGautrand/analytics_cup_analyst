...
class PhasesOfPlayAggregator:
    ...
    def _add_team_out_of_possession_info(self):
        ...
        self.phases_of_play_df["team_out_of_possession_id"] = self.phases_of_play_df[
            "team_in_possession_id"
        ].map(id_mapping)
        self.phases_of_play_df[
    ...
    def _add_next_phase(self):
        ...
        """
        self.phases_of_play_df = self.phases_of_play_df.sort_values(
            by=["frame_start"]
        ...
        frame_start_to_index = {
            self.phases_of_play_df.loc[i, "frame_start"]: i
            for i in range(len(self.phases_of_play_df))
        }
        ...
        for i in range(len(self.phases_of_play_df) - 1):
            ...
            if current_frame_end in frame_start_to_index:
                ...
                if (
                    self.phases_of_play_df.loc[i, "team_in_possession_id"]
                    == self.phases_of_play_df.loc[next_idx, "team_in_possession_id"]
                ):
                    self.phases_of_play_df.loc[
                        i, "team_in_possession_next_phase"
                    ] = self.phases_of_play_df.loc[
                        next_idx, "team_in_possession_phase_type"
                    ]
                else:
                    self.phases_of_play_df.loc[
                        i, "team_in_possession_next_phase"
                    ] = self.phases_of_play_df.loc[
                        next_idx, "team_out_of_possession_phase_type"
                    ]

                if (
                    self.phases_of_play_df.loc[i, "team_out_of_possession_id"]
                    == self.phases_of_play_df.loc[next_idx, "team_out_of_possession_id"]
                ):
                    self.phases_of_play_df.loc[
                        i, "team_out_of_possession_next_phase"
                    ] = self.phases_of_play_df.loc[
                        next_idx, "team_out_of_possession_phase_type"
                    ]
                else:
        ...
        self.phases_of_play_df.loc[
            self.phases_of_play_df["team_in_possession_next_phase"].isnull(),
            "team_in_possession_next_phase",
        ] = "no_next_phase"
        self.phases_of_play_df.loc[
            self.phases_of_play_df["team_out_of_possession_next_phase"].isnull(),
    ...
    def get_out_of_possession_aggregates(self):
        ...
        out_of_possession_phase_aggs = (
            self.phases_of_play_df.groupby(group_by)
            .agg(
        ...
        next_phase_in_possession_phase_aggs = (
            self.phases_of_play_df.groupby(
                [
                    "team_out_of_possession_id",
                    "team_out_of_possession_phase_type",
                    "team_out_of_possession_next_phase",
        ...
        next_phase_in_possession_phase_aggs["team_phase_id"] = (
            next_phase_in_possession_phase_aggs["team_out_of_possession_id"].astype(str)
            + "_"
            + next_phase_in_possession_phase_aggs["team_out_of_possession_phase_type"]
        )

        out_of_possession_phase_aggs["team_phase_id"] = (
            out_of_possession_phase_aggs["team_out_of_possession_id"].astype(str)
            + "_"
            + out_of_possession_phase_aggs["team_out_of_possession_phase_type"]
        )

        for next_phase in next_phase_in_possession_phase_aggs[
            "team_out_of_possession_next_phase"
        ].unique():
            next_phase_df = next_phase_in_possession_phase_aggs[
                next_phase_in_possession_phase_aggs["team_out_of_possession_next_phase"]
                == next_phase
            ]

            next_phase_mapping = dict(
                zip(next_phase_df["team_phase_id"], next_phase_df["count"])
            )

            out_of_possession_phase_aggs[
                f"count_into_{next_phase}_from"
        ...
        out_of_possession_phase_aggs.columns = [
            "{}_{}".format(m, c) for m, c in out_of_possession_phase_aggs.columns
        ]
        ...
        for phase in [
            ...
            for metric in [
                ...
                "count_possession_lost_in_phase",
                "count_possession_lead_to_shot",
                "count_possession_lead_to_goal",
                "avg_start_width",
                "avg_start_length",
                "avg_end_width",
                "avg_end_length",
                "count_into_build_up_from",
                "count_into_create_from",
                "count_into_finish_from",
                "count_into_transition_from",
                "count_into_quick_break_from",
                "count_into_direct_from",
                "count_into_chaotic_from",
                "count_into_low_block_from",
                "count_into_medium_block_from",
                "count_into_high_block_from",
                "count_into_defending_quick_break_from",
                "count_into_defending_transition_from",
    ...
    def get_in_possession_aggregates(self):
        ...
        next_phase_in_possession_phase_aggs["team_phase_id"] = (
            next_phase_in_possession_phase_aggs["team_in_possession_id"].astype(str)
            + "_"
            + next_phase_in_possession_phase_aggs["team_in_possession_phase_type"]
        )

        in_possession_phase_aggs["team_phase_id"] = (
            in_possession_phase_aggs["team_in_possession_id"].astype(str)
            + "_"
        ...
        for next_phase in next_phase_in_possession_phase_aggs[
            ...
            next_phase_df = next_phase_in_possession_phase_aggs[
                next_phase_in_possession_phase_aggs["team_in_possession_next_phase"]
                == next_phase
            ]
        ...
        in_possession_phase_aggs.columns = [
            "{}_{}".format(m, c) for m, c in in_possession_phase_aggs.columns
        ]
